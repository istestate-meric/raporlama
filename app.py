...
if selected_keys:
    ...
    for idx, fonk_adi in enumerate(unique_active_functions):
        with func_cols[idx % len(func_cols)]:
            ...
            # DÜZELTME: 'allowed_types' yerine doğru değişken adı 'allowed_p_types' kullanıldı
            default_type_idx = allowed_p_types.index(best_type) if best_type in allowed_p_types else 0
            
            sub_col1, sub_col2 = st.columns(2)
            with sub_col1:
                selected_func_p_type = st.selectbox(
                    f"Proje Tipi (En Kârlı Seçildi)", 
                    options=allowed_p_types, 
