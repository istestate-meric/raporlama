...
if selected_keys:
    ...
    with tab5:
        ...
        # --- ESTETİK VE KURUMSAL PDF ŞABLONU (Logolu & Gelişmiş CSS) ---
        pdf_logo1_html = f"<img src='data:image/png;base64,{img1_base64}' style='max-height: 55px; width: auto;'/>" if img1_base64 else "<b>İSTESTATE</b>"
        pdf_logo2_html = f"<img src='data:image/png;base64,{img2_base64}' style='max-height: 55px; width: auto;'/>" if img2_base64 else "<b>MERİÇ İNŞAAT</b>"

        arsa_sahibi_row_html = ""
        if "Kat Karşılığı" in is_modeli:
            arsa_sahibi_row_html = f"""
                    <tr>
                        <td style="padding: 9px 12px; border-bottom: 1px solid #e2e8f0; color: #334155;">Arsa Sahibi Payı</td>
                        <td style="padding: 9px 12px; border-bottom: 1px solid #e2e8f0; text-align: right; color: #334155;">${arsa_sahibi_payi_usd:,.2f}</td>
                        <td style="padding: 9px 12px; border-bottom: 1px solid #e2e8f0; text-align: right; color: #334155;">-</td>
                    </tr>
            """
        ...
        <div class="report-wrapper">
            ...
            <!-- KURÜMSAL HEADER VE LOGOLAR -->
            <table class="header-table">
                <tr>
                    <td style="width: 45%;">
                        <div style="text-align: left;">
                            <span style="display: inline-block; margin-right: 15px;">{pdf_logo1_html}</span>
                            <span style="display: inline-block;">{pdf_logo2_html}</span>
                        </div>
                    </td>
                    <td style="width: 55%;" class="title-area">
                        <h1 class="brand-title">İSTESTATE & MERİÇ İNŞAAT</h1>
                        <p class="brand-sub">Akıllı Gayrimenkul Geliştirme ve Fizibilite Raporu</p>
                    </td>
                </tr>
            </table>
            
            <h3>1. Proje ve Lokasyon Künyesi</h3>
            <div class="content-row"><b>Seçilen Lokasyon / Mahalle:</b> {detected_mahalle}</div>
            <div class="content-row"><b>Proje Tipi:</b> {selected_proje_tipi}</div>
            <div class="content-row"><b>İş Modeli:</b> {is_modeli}</div>
            <div class="content-row"><b>Toplam Brüt İnşaat Alanı:</b> {yasal_max_brut_insaat_alani:,.2f} m²</div>
            
            <h3>2. Mimari ve Bağımsız Bölüm Planlaması</h3>
            <div class="content-row"><b>Bağımsız Bölüm / Villa Adedi:</b> {curr_hb} Adet</div>
            <div class="content-row"><b>Havuz Planlama Modeli:</b> {curr_hp}</div>
            <div class="content-row"><b>Ortalama Ünite Brüt Alanı:</b> {pdf_unite_metin}</div>
            ...
            <table class="data-table">
                ...
                <tbody>
                    <tr>
                        <td style="padding: 9px 12px; border-bottom: 1px solid #e2e8f0; color: #334155;">Toplam Tahmini Brüt Ciro</td>
                        <td style="padding: 9px 12px; border-bottom: 1px solid #e2e8f0; text-align: right; color: #334155;">${toplam_ciro_usd:,.2f}</td>
                        <td style="padding: 9px 12px; border-bottom: 1px solid #e2e8f0; text-align: right; color: #334155;">₺{toplam_ciro_tl:,.2f}</td>
                    </tr>
                    <tr>
                        <td style="padding: 9px 12px; border-bottom: 1px solid #e2e8f0; color: #334155;">Toplam İnşaat Maliyeti + Bonus</td>
                        <td style="padding: 9px 12px; border-bottom: 1px solid #e2e8f0; text-align: right; color: #334155;">${toplam_maliyet_usd:,.2f}</td>
                        <td style="padding: 9px 12px; border-bottom: 1px solid #e2e8f0; text-align: right; color: #334155;">₺{toplam_maliyet_tl:,.2f}</td>
                    </tr>
                    {arsa_sahibi_row_html}
                    <tr style="background-color: #f1f5f9; font-weight: bold;">
                        <td style="padding: 10px 12px; color: #0f172a;">Müteahhit Net Kârı</td>
                        <td style="padding: 10px 12px; text-align: right; color: #0f172a;">${mutaahhit_net_kar_usd:,.2f}</td>
                        <td style="padding: 10px 12px; text-align: right; color: #0f172a;">₺{mutaahhit_net_kar_tl:,.2f} (%{yg_orani:.1f} YG)</td>
                    </tr>
                </tbody>
            </table>
    ...
    st.info("👋 **Hoş Geldiniz!** Raporları görüntülemek için lütfen sol menüden bir **Ada** seçip ilgili parselleri işaretleyin veya yeni bir imar belgesi (PDF) yükleyin.")import re
import os
import json
import base64
import urllib.request
